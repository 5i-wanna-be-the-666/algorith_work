...
Session session = null;
	...
	public UserRole findById(java.lang.String id) {
		...
		try {
			UserRole instance = (UserRole) getSession().get("user_role.UserRole", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(UserRole instance) {
		...
		try {
			List results = getSession().createCriteria("user_role.UserRole").add(Example.create(instance)).list();
